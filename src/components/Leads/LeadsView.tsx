...
export default function LeadsView({ organizationId }: LeadsViewProps) {
  ...
  const leads = useQuery(api.projects.getByStatus, {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter(lead => {
      ...
      return (
        ...
        serviceTypeLabels[lead.serviceType]?.toLowerCase().includes(query) ||
        lead.projectIntent?.toLowerCase().includes(query)
      );
    });
  }, [leads, customers, searchQuery]);
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Box>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              Manage incoming leads and qualify prospects
            </Typography>
          </Box>
          <Button
            variant="contained"
        ...
        <TextField
          fullWidth
          placeholder="Search leads by customer, address, or service type..."
          value={searchQuery}
                  ...
                  <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Chip
                          label={serviceTypeLabels[lead.serviceType] || lead.serviceType}
                          size="small"
                          sx={{
                            bgcolor: '#3b82f6',
                            color: '#fff',
                    ...
                    {getCustomerAddress(lead.customerId) && (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                    ...
                    {lead.projectIntent && (
                      <Typography variant="body2" sx={{ color: 'text.secondary', mb: 1, fontSize: '0.875rem' }}>
                        Intent: {lead.projectIntent}
                      </Typography>
                    )}

                    {lead.treeShopScore !== undefined && (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                          TreeShop Score:
                        </Typography>
                        <Typography variant="body2" sx={{ fontWeight: 600, color: 'primary.main' }}>
                          {lead.treeShopScore.toFixed(1)}
                        </Typography>
                      </Box>
                    )}
      ...
      >
        <MenuItem onClick={handleConvertToProposal}>
          Convert to Proposal
        </MenuItem>
